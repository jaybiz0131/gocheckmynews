...
def run(mode="live", fixture=None):
    ...
    try:
        ...
        # `attempted` is what the failure message reports. report["stages"] only ever holds
        # stages that SUCCEEDED, because record() runs after the call returns, so reading
        # stages[-1] on the way out names the last stage that worked rather than the one that
        # broke. A live run died in the approver on 2026-07-31 and the alert said
        # "FAILED at 4-writer", one line after the writer had printed "drafted 8/8".
        attempted = "2-editor"
